#!/bin/bash
filePath='/home/kafka/ES_new.csv'
...
then
 ...
 echo "Date , ES_PID ,ES_CPU ,ES_RSSMemory ,ES_VirtualMemory ,ES_RSSMemory_Percent ,ES_Heap_USED, ES_Heap_Total,ES_DiskRead ,ES_DiskWrite, us_CPU , sy_CPU, ni_CPU,idle_CPU,wa_CPU,hi_CPU,si_CPU,st_CPU, Machine_TotalMemUsage ,Machine_ActualMemUsage ,Machine_TotalMem ,Machine_DiskUsage, Machine_DiskUsage_percent,Machine_DiskRead, Machine_DiskWrite
" >> $filePath
fi
...
PIDS=$(ps ax -o pid,rssize,vsize,pcpu,command | grep elasticsearch-2.2.1/lib | grep -v grep |awk '{print $1}')
...
RSS_MM1=$(ps ax -o pid,rssize,vsize,pcpu,command | grep elasticsearch-2.2.1/lib | grep -v grep  |awk '{print $2}')
RSS_MM=$(echo "$RSS_MM1/1024"| bc -l | awk '{printf "%f", $0}')
VM_MM1=$(ps ax -o pid,rssize,vsize,pcpu,command | grep elasticsearch-2.2.1/lib | grep -v grep  |awk '{print $3}')
VM_MM=$(echo "$VM_MM1/1024"| bc -l | awk '{printf "%f", $0}')
#CPU_P=$(ps ax -o pid,rssize,vsize,pcpu,command | grep elasticsearch-2.2.1/lib | grep -v grep |awk '{print $4}')
...
# Memory Usage
MEM_PR=$(ps aux | grep elasticsearch-2.2.1/lib  |grep $mm_pid1 | grep -v grep |awk '{print $4}')
